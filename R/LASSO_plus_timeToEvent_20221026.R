...
#' LASSO_plus on variable selection for time to event outcome
...
#' @param topN An interger indicate how many variables we intend to select 
...
LASSO_plus_binary = function(data, biomks,  time, event, topN = 10){
  ...
  surObj = Surv(time, event)
  
  lassoF = glmnet(x= data.matrix(dat[,vars]), y= surObj, family = "cox")
...
#   dev.off()
  ...
  ###############################################################################
  survY = paste0("Surv(", morevar[1],",", morevar[2], ")")
  cox1out = t(sapply(vars, function(var1){
    out1 = coxph(as.formula(paste(survY, var1, sep=" ~ ")), data = data)
  ...
  # if(dim(cox1out)[1] > 0){ 
  #   cox1out = head(cox1out, n = topN)
  #   write.csv(cox1out, gsub("LASSO", "singleVar", lassoCoef))
  # }
  # 
  ...
  #fit1 = glm(as.formula(paste(Y, vX, sep=" ~ ")), family="binomial",data = dat)
  ...
  ###########
  ## stop here on 20221026, should refer to coxTimeToEvent_20220928.R to get same output to the main function
  #########
