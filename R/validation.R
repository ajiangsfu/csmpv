...
#' 
#' @description This function is designed to perform model validation when the outcome variable is available. It facilitates the comparison of model predictions with the provided outcome variable, which can be continuous, binary, or time-to-event.
#' 
#' @details This function is invoked by multiple prediction functions within this package when an outcome variable is available for a new dataset. However, users can also directly call this function if needed. While comparison...
#' 
#' @param predicted A vector of model prediction values, which can be generated by prediction functions in this package such as LASSO_predict and XGBtraining_predict. For continuous outcomes, this vector represents model scores; for binary outcomes, it represents the probability of the positive class; for time-to-event outcomes, it contains risk scores, which will later be transformed into estimated survival probabilities corresponding to times in the new data.
#'

#' @param outcomeType Outcome variable type. There are three choices: "binary" (default), "continuous", and "time-to-event".
#' @param trueY A vector of outcome variable when it is continuous or binary
#' @param time A vector of time for time to event outcome
#' @param trueEvent A vector of event for time to event outcome
#' @param baseHz A table for accumulate baseline hazard for multiple time points, which is usually generated based on a training data set. 
...
#' 
#' @return A vector of model prediction values from input
...
validation = function(predicted =NULL, outcomeType = c("binary","continuous","time-to-event"), trueY = NULL, time = NULL, trueEvent = NULL, 
  ...
  if(outcomeType == "continuous"){
    ...
    ## the trick is how to get matched CumHaz, should write a function to do so
    #ss = get_matched_values (existing_data = baseHz, search_var = "time", target_var = "hazard", new_data = time)
    ## notice that, right now, ss just matched hazard at each time point
    
    ## should work on single time point just as from survest function,
    ## and if no exactly matching, should use the longer time
    ...
    ## "must specify fit if u is specified and fun is not" => since we do not specify u, should not set u
    ##print(cindex)
    pdf(paste0(outfile, "_newData_validation.pdf"))
